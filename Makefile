...
index:
	@if ( [ -f "$(INDEX_ROOT)/library.json" ] && [ -s "$(INDEX_ROOT)/library.json" ] ) || \
	   ( [ -f "$(INDEX_ROOT)/data.json" ] && [ -s "$(INDEX_ROOT)/data.json" ] ); then \
		echo "Index present: $(INDEX_ROOT)"; \
		exit 0; \
	fi; \
	PDF_COUNT=$$(find "$(PDF_DIR)" -maxdepth 1 -name "*.pdf" 2>/dev/null | wc -l); \
	if [ "$$PDF_COUNT" -eq 0 ]; then \
		echo "No PDFs found. Add PDFs to ./$(PDF_DIR) then run \`make index\` or \`make run-bootstrap\`."; \
		exit 0; \
	fi; \
