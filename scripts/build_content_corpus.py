...
def clean_text(text: str) -> str:
   ...
   for line in text.split('\n'):
      ...
      # Remove dot-leader lines (lines containing spaced/consecutive dots + page number)
      # Use .search() since dots can appear anywhere in the line
      stripped = line.strip()
      if stripped and _DOT_LEADER_RE.search(stripped):
...
def check_filters(
   ...
   if not config.include_noncontent and page_classifications:
      for pg in range(page_start, page_end + 1):
         ...
         if cls and cls.get('page_type') in NONCONTENT_PAGE_TYPES:
   ...
   # --- 4. Section title is dot-padded TOC entry ---
...
def build_corpus(
   ...
   with open(sections_path, 'r', encoding='utf-8') as fin, \
      ...
      for line in fin:
         ...
         # ── Build provenance ─────────────────────────────────────────
         page_types = get_page_types_in_range(page_start, page_end, page_cls)
